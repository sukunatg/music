...
pkkasbeiwvi["t_btns"] = [(0, 0), (1, 1), (1, 0), (0, 1)]
...
@H1.on(telethon.events.NewMessage(pattern = "/togglepoke", outgoing = True))
...
async def challenge_main_clicker(event):
    try:
        ...
        if not event.sender_id in POKA_BOTS_LIST:
            return
        pkkasbeiwvi["number_of_edits"] += 1
        if pkkasbeiwvi["number_of_edits"] % 2:
            await event.click(*get_pref())
        else:
