...
if Config.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id in auth and query == "hellbot_help":
            ...
            if a:
                ...
                help_pic = "https://graph.org/file/03005c65cae97edbceea3.mp4"
            ...
            alive_msg = gvarstat("ALIVE_MSG") or "»»» <b>ProximaUserBot is Online</b> «««"
            ...
            if a and a == "DISABLE":
                ...
                PIC = "https://graph.org/file/9e1a4a784bd83cf10bd62.jpg"
            if PIC and PIC.endswith((".jpg", ".png")):
                result = builder.photo(
                    file=PIC,
                    text=HELL_FIRST,
            ...
            result = builder.article(
                title="About",
                text=f"**✧･ﾟ: 𝘐𝘵𝘴 𝘗𝘳𝘰𝘹𝘪𝘮𝘢 𝘜𝘴𝘦𝘳𝘉𝘰𝘵 𝘏𝘦𝘳𝘦 :･ﾟ✧**",
                buttons=[
                    [Button.url("📑 About 📑", "https://graph.org/PROXIMA-USERBOT-05-26")],
                ],
            )
            ...
            result = builder.article(
                "Proxima!",
                text="""**Hey! This is [PROXIMA](https://graph.org/PROXIMA-USERBOT-05-26) \nHell there!**""",
                buttons=[
                    [
                        custom.Button.url("🔥 ABOUT 🔥", "https://graph.org/PROXIMA-USERBOT-05-26"),
                    ],
                    [
                        custom.Button.url(
                            "✨ ABOUT ✨", "https://graph.org/PROXIMA-USERBOT-05-26"
                        ),
                        custom.Button.url(
                            "🔰 TUTORIAL 🔰", "https://graph.org/PROXIMA-USERBOT-05-26"
                        ),
                    ],
                ],
                link_preview=False,
    ...
    async def on_pm_click(event):
        ...
        if event.query.user_id in auth:
            ...
            reply_pop_up_alert = "🔰 This is ProximaUserBot PM Security to keep away unwanted retards from spamming PM !!"
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        if event.query.user_id in auth:
            ...
            await event.edit(
                f"**⚜️ ProximaUserBot Menu Provider is now Closed ⚜️**\n\n**Bot Of :**  {hell_mention}\n\n        [©️ Proximauserbot ™️]",
                buttons=veriler,
                link_preview=False,
            )
        else:
