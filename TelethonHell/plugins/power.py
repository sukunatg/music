...
async def restart(event):
    if Config.HEROKU_APP_NAME and Config.HEROKU_API_KEY:
        ...
        await eor(event, f"✅ **ʀᴇꜱᴛᴀʀᴛᴇᴅ ᴘʀᴏxɪᴍᴀᴜʙ** \n**Type** `{hl}ping` **after 1 minute to check if I am working !**")
        await event.client.disconnect()


@hell_cmd(pattern="restart$")
async def re(hell):
    event = await eor(hell, "Restarting ProximaUserBot ...")
    try:
        await restart(event)
...
async def rel(event):
    await eor(event, "**Reloaded ProximaBot!** \n\n__This might take a minute.__")
    await reload_hellbot()


@hell_cmd(pattern="shutdown$")
async def down(event):
    await eor(event, "**[ ⚠️ ]** \n**ProximaUserBot is now turned off. Manually turn it on to start again.**")
